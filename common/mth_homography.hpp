...
public:
  ...
  T			hinv[9];	// inverse of 'hi'
private:
  T			dumh[9];	// homography data array (NOT FOR DIRECT REFERENCE)
...
public:
  ...
  Homography(T *data) : h(data) { 
    H.set(3,3,h); 
    MTX::MatrixSolver<T> solver;
    solver.inverseByDeterminant( 3, h, hinv );
  };
  ~Homography() {};
  inline bool isReady(void) { return (h && h[8]==1); }
...
private:
  ...
  // -----------------------------------------------------------------
public:
  inline void transform(T p[2], T q[2]) { transform( p[0], p[1], q ); }
  ...
  inline void transformInverse(T q[2], T p[2]) { transformInverse( q[0], q[1], p ); }
  void transformInverse(T qx, T qy, T p[2]) {
    // w * q = H * p  =>  w' * p = Hinv * q
    T  w  =  hinv[6] * qx + hinv[7] * qy + hinv[8];
    p[0]  = (hinv[0] * qx + hinv[1] * qy + hinv[2]) / w;
    p[1]  = (hinv[3] * qx + hinv[4] * qy + hinv[5]) / w;
  }
...
public:
  void printInfo(const char *title=NULL, T p0[2]=NULL, T p1[2]=NULL, T p2[2]=NULL) {
    ...
    printf("%s [ %10.4f %10.4f %10.4f ]", title, h[0], h[1], h[2]);
    if (p0) printf("   xy(%4.1f %4.1f) -> uv(%5.1f %5.1f) \n", p0[0], p0[1], q0[0], q0[1]);
    else printf("\n");
    printf("%s [ %10.4f %10.4f %10.4f ]", blank, h[3], h[4], h[5]);
    if (p1) printf("   xy(%4.1f %4.1f) -> uv(%5.1f %5.1f) \n", p1[0], p1[1], q1[0], q1[1]);
    else printf("\n");
    printf("%s [ %10.4f %10.4f %10.4f ]", blank, h[6], h[7], h[8]);
    if (p2) printf("   xy(%4.1f %4.1f) -> uv(%5.1f %5.1f) \n", p2[0], p2[1], q2[0], q2[1]);
    else printf("\n");
    //printf("%s [ %10.4f %10.4f %10.4f ]\n", "      hinv", hinv[0], hinv[1], hinv[2]);
    //printf("%s [ %10.4f %10.4f %10.4f ]\n",        blank, hinv[3], hinv[4], hinv[5]);
    //printf("%s [ %10.4f %10.4f %10.4f ]\n",        blank, hinv[6], hinv[7], hinv[8]);
  }
