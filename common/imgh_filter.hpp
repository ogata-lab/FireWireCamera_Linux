...
public:
  void convoluteWithGaussian(IMGH::Image *src, IMGH::Image *dst=NULL, 
    ...
    // Note that 'src' and 'dst' can be the same image.
...
//   void getMinMaxWithAlpha(IMGH::Image *src, unsigned char *dst, int wsize, bool for_max) {
//     // Note that 'src' and 'dst' can be same.
//     if (!src || src->type != PIXEL_RGB) return;
//     int i, j, pos;
//     if (wsize < 1) wsize = 1;
//     if (wsize%2 == 0) wsize++;	// window size must be odd.
//     // get min/max value along x axis, and save the result in 'tmp'
//     unsigned char *tmp = (unsigned char*) malloc (src->w * src->h * sizeof(unsigned char));
//     for (i = 0; i < src->h; i++) {
//       pos = i * src->w;
//       getMinMaxWithBuffer(src->alpha_data+pos, tmp+pos, wsize, src->w, 1, for_max);
//     }
//     // get min/max value along y axis, and save the result in 'dst'
//     for (j = 0; j < src->w; j++) {
//       getMinMaxWithBuffer(tmp+j, dst+j, wsize, src->h, src->w, for_max);
//     }
//     free(tmp);
//   }
  ...
  // -----------------------------------------------------------------
public:
  void convoluteWithBox(IMGH::Image *src, IMGH::Image *dst=NULL, 
			int filter_size=3, bool avg=true, 
			behavior_t behavior=IMGH_FILTER_REPEAT) {
    this->createBoxFilter( filter_size, (avg ? 1.0 : 0.0) );
    this->convolute( src, dst, behavior );
  }
  void createBoxFilter(int size=5, double scale=0.0) {
    // Create a box filter into the member variable 'mask'.
    //   scale : scale of the filter (0: sum-up filter, 1:average filter)
    clear();
    if (size <= 1) return;
    else if (size % 2 == 0) size++;
    mask = (float*) malloc(size * sizeof(float));	
    for (int i=0; i<size; i++) mask[i] = 1.0; 	// calculate mask values
    if (scale > 0) {
      double norm = size / scale;		// normalize
      for (int i=0; i<size; i++) mask[i] /= (float)norm;
    }
  }
  
  void avgImage(IMGH::Image *src, IMGH::Image *dst=NULL,
		int wsize=3, double scale=1, behavior_t behavior=IMGH_FILTER_REPEAT) {
    // Sum up pixel values with a window. (without setting up the mask)
    //   scale : scale for the final pixel value (0: sum-up, 1:average)
    // Note that 'src' and 'dst' can be same.
    if (!src || !mask) return;
    int         i, j, pos;
    IMGH::Image tmp( src->w, src->h, src->type );
    ...
    case PIXEL_GRAY: 
      for (i = 0; i < src->h; i++) {	// convolution along X axis (src -> tmp)
	pos = i * src->w;
	ccv.average(src->data+pos, tmp.data+pos, wsize, src->w, 1, behavior);
      }
      for (j = 0; j < src->w; j++)	// convolution along Y axis (tmp -> dst)
	ccv.average(tmp.data+j, dst->data+j, wsize, src->h, src->w, behavior);
      break;
//     case PIXEL_RGB:
//       for (i = 0; i < src->h; i++) {	// convolution along X axis (src -> tmp)
// 	pos = i * 3 * src->w;
// 	ccv.average3(src->data+pos, tmp.data+pos, wsize, src->w, 3, behavior);
//       }
//       for (j = 0; j < src->w; j++)	// convolution along Y axis (tmp -> dst)
// 	ccv.average3(tmp.data+j*3, dst->data+j*3, wsize, src->h, 3*src->w, behavior);
//       break;
    case PIXEL_FLOAT:
      for (i = 0; i < src->h; i++) {	// convolution along X axis (src -> tmp)
	pos = i * src->w;
	fcv.average(src->data+pos, tmp.data+pos, wsize, src->w, 1, behavior);
      }
      for (j = 0; j < src->w; j++)	// convolution along Y axis (tmp -> dst)
	fcv.average(tmp.data+j, dst->data+j, wsize, src->h, src->w, behavior);
      break;
    default: break;
    }
  }
