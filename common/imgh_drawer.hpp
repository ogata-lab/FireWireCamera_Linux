...
public:
  ImageDrawer(IMGH::Image *img) : r(255), g(255), b(255), line_thickness(1), imgp(NULL) {
    imgp = img;
  }
  ImageDrawer(int w, int h, pixel_t type, void *buffer) : line_thickness(1) { 
...
public:
  ...
  void setColorRandomly(void) { 
    r = (unsigned char)(255.0f*rand()/RAND_MAX); 
    g = (unsigned char)(255.0f*rand()/RAND_MAX); 
    b = (unsigned char)(255.0f*rand()/RAND_MAX); 
  }
...
public:
  ...
  void setPixelWithRandomColor(int idx) {
    switch (imgp->type) {
    case PIXEL_GRAY: imgp->setChar(idx, 50+rand()%206);  break;
    case PIXEL_RGB:  imgp->setRGB (idx, 50+rand()%206, 50+rand()%206, 50+rand()%206);  break;
    case PIXEL_BGR:  imgp->setRGB (idx, 50+rand()%206, 50+rand()%206, 50+rand()%206);  break;
    default: break;
    }
  }
  ...
  void drawPixel(T_t x, T_t y, int r=-1, int g=-1, int b=-1, bool check_bd=true) {
    ...
    if (check_bd) {
      ...
      case PIXEL_BGR:  imgp->setRGB (idx, b, g, r );  break;
      case PIXEL_RGBA: imgp->setRGB (idx, r, g, b );  break;
      default: std::cout << "Error (IMGH::ImageDrawer::drawLine): invalid pixel type" << std::endl;  break;
      }
    }
  }
  ...
  void drawTriangle(T_t p0[2], T_t p1[2], T_t p2[2], int r=-1, int g=-1, int b=-1, bool check_bd=true) {
    drawLine( p0[0], p0[1],  p1[0], p1[1],  r, g, b,  check_bd );
    drawLine( p1[0], p1[1],  p2[0], p2[1],  r, g, b,  check_bd );
    drawLine( p2[0], p2[1],  p0[0], p0[1],  r, g, b,  check_bd );
  }
  void drawTriangle(T_t x0, T_t y0, T_t x1, T_t y1, T_t x2, T_t y2,
		    int r=-1, int g=-1, int b=-1, bool check_bd=true) {
    drawLine( x0, y0,  x1, y1,  r, g, b,  check_bd );
    drawLine( x1, y1,  x2, y2,  r, g, b,  check_bd );
    drawLine( x2, y2,  x0, y0,  r, g, b,  check_bd );
  }
  ...
  void drawFilledRectangle(int x, int y, int w, int h, int r, int g, int b, int a=-1) {
    uchar_t rgba[4];
    if (x+w >= imgp->w) w = imgp->w - x;
    if (y+h >= imgp->h) h = imgp->h - y;
    for (int j = y; j < y+h; j++) {
      for (int i = x; i < x+w; i++) {
	imgp->getPixel( i, j, rgba );
	if (r >= 0) rgba[0] = r;  if (g >= 0) rgba[1] = g;
	if (b >= 0) rgba[2] = b;  if (a >= 0) rgba[3] = a;
	imgp->setPixel( i, j, rgba );
      }
    }
  }
  ...
  void drawTransparentQuad(T_t p0[2], T_t p1[2], T_t p2[2], T_t p3[2], 
			   double opaque, int r=-1, int g=-1, int b=-1, bool check_bd=true) {
    drawTransparentLine( p0[0], p0[1],  p1[0], p1[1],  opaque, r, g, b,  check_bd );
    drawTransparentLine( p1[0], p1[1],  p2[0], p2[1],  opaque, r, g, b,  check_bd );
    drawTransparentLine( p2[0], p2[1],  p3[0], p3[1],  opaque, r, g, b,  check_bd );
    drawTransparentLine( p3[0], p3[1],  p0[0], p0[1],  opaque, r, g, b,  check_bd );
  }
  void drawTransparentQuad(T_t x0, T_t y0, T_t x1, T_t y1, T_t x2, T_t y2, T_t x3, T_t y3, 
			   double opaque, int r=-1, int g=-1, int b=-1, bool check_bd=true) {
    drawTransparentLine( x0, y0,  x1, y1,  opaque, r, g, b,  check_bd );
    drawTransparentLine( x1, y1,  x2, y2,  opaque, r, g, b,  check_bd );
    drawTransparentLine( x2, y2,  x3, y3,  opaque, r, g, b,  check_bd );
    drawTransparentLine( x3, y3,  x0, y0,  opaque, r, g, b,  check_bd );
  }
  ...
  void drawCross(T_t x0, T_t y0, int size, char type='+',
		 int r=-1, int g=-1, int b=-1, bool check_bd=true) {
    int x = ROUNDI(x0),  y = ROUNDI(y0), dx=0, dy=0; 
    if        (type == '+') {
      drawLine( x-size, y,  x+size, y,  r, g, b,  check_bd );
      drawLine( x, y-size,  x, y+size,  r, g, b,  check_bd );
    } else if (type == 'x') {
      drawLine( x-size, y-size,  x+size, y+size,  r, g, b,  check_bd );
      drawLine( x+size, y-size,  x-size, y+size,  r, g, b,  check_bd );
    } else if (type=='|') {
      drawLine(x-size/2, y, x+size/2, y,  r, g, b,  check_bd); 
      drawLine(x, y-size, x, y+size,  r, g, b,  check_bd); 
    } else if (type=='-') {
      drawLine(x-size, y, x+size, y,  r, g, b,  check_bd); 
      drawLine(x, y-size/2, x, y+size/2,  r, g, b,  check_bd); 
    } else {
      switch (type) {
      case '0':  dx = +size;  dy = -size;  break;  // corner L
      case '1':  dx = -size;  dy = -size;  break;  // corner J
      case '2':  dx = -size;  dy = +size;  break;  // corner 7
      case '3':  dx = +size;  dy = +size;  break;  // corner F
      }
      drawLine( x, y,  x+dx, y,  r, g, b,  check_bd );
      drawLine( x, y,  x, y+dy,  r, g, b,  check_bd );
    }
  }
