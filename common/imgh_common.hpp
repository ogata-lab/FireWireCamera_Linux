...
//   bool copyChannelFromImage( &src_image, 'A' );	// 'R', 'G', 'B', or 'A'
...
#define IMGH3V_MINV(a)        ( (a)[0]<(a)[1] ? ((a)[0]<(a)[2] ? (a)[0] : (a)[2]) : ((a)[1]<(a)[2] ? (a)[1] : (a)[2]) )
#define IMGH2V_MAX( a, b )    ( (a) > (b) ? (a) : (b) )
#define IMGH3V_MAX( a, b, c ) ( (a) > (b) ? ((a) > (c) ? (a) : (c)) : ((b) > (c) ? (b) : (c)) )
#define IMGH3V_MAXV(a)        ( (a)[0]>(a)[1] ? ((a)[0]>(a)[2] ? (a)[0] : (a)[2]) : ((a)[1]>(a)[2] ? (a)[1] : (a)[2]) )
...
public:
  ...
  Image (int w, int h, pixel_t type=PIXEL_RGB, bool clear_image=false) 
    : w(0), h(0), type(PIXEL_UNKNOWN), data(NULL), allocated(false), row_size(0), pixel_size(0) { 
    setImage(w, h, type, clear_image); 
  }
/*  Image (int w, int h, pixel_t type=PIXEL_RGB, void *buffer, bool clear_image=false) 
    : w(0), h(0), type(PIXEL_UNKNOWN), data(NULL), allocated(false), row_size(0), pixel_size(0) { 
    setImage(w, h, type, buffer, clear_image);
  }*/
  Image (Image *src) : w(0), h(0), data(NULL), allocated(false) { copyFrom(src); }
  ~Image() { clear(); }
  inline void clear(void)   { if (allocated&&data) free(data); data=NULL; w=h=0; type=PIXEL_UNKNOWN; }
  inline bool isReady(void) { return (type!=PIXEL_UNKNOWN && data!=NULL); }
...
public:
  ...
  inline void getRGB(int x, int y, int rgb[3]) { uchar_t *p = data+(y*w+x)*pixel_size; IMGH3V_COPY( rgb, p ); }
  inline void getRGB(int idx,      int rgb[3]) { uchar_t *p = data+ (idx) *pixel_size; IMGH3V_COPY( rgb, p ); }
  inline void setRGB(int x, int y, int rgb[3]) { uchar_t *p = data+(y*w+x)*pixel_size; IMGH3V_COPY( p, rgb ); }
  inline void setRGB(int idx,      int rgb[3]) { uchar_t *p = data+ (idx) *pixel_size; IMGH3V_COPY( p, rgb ); }
  ...
  inline uchar_t getChannel(int idx     , int ch) { return (((uchar_t*)data)+ (idx) *pixel_size)[ch]; }
  inline uchar_t getChannel(int x, int y, int ch) { return (((uchar_t*)data)+(w*y+x)*pixel_size)[ch]; }
  inline void    setChannel(int idx     , int ch, uchar_t v) { (((uchar_t*)data)+ (idx) *pixel_size)[ch] = v; }
  inline void    setChannel(int x, int y, int ch, uchar_t v) { (((uchar_t*)data)+(w*y+x)*pixel_size)[ch] = v; }
  inline int getGrayFromRGB(int idx)      { int r,g,b; getRGB(idx,&r,&g,&b); return ((30*r + 59*g + 11*b)/100); }
  inline int getGrayFromRGB(int x, int y) { int r,g,b; getRGB(x,y,&r,&g,&b); return ((30*r + 59*g + 11*b)/100); }
  
  // -----------------------------------------------------------------

  bool copyFrom(IMGH::Image *img) { 
  ...
  bool copyChannelFromImage(Image *src, int ch) {
    if (!src) return false;
    setImage( src->w, src->h, PIXEL_GRAY );
    int   i, total = w*h, s = src->pixel_size;
    if      (ch < 4) {}
    else if (src->type==PIXEL_GRAY)  ch = 0;
    else if (src->type==PIXEL_GRAYA) ch = (ch=='A' ? 1 : 0);
    else if (src->type==PIXEL_RGB)   ch = (ch=='R' ? 0 : (ch=='G' ? 1 : 2));
    else if (src->type==PIXEL_RGBA)  ch = (ch=='R' ? 0 : (ch=='G' ? 1 : (ch=='B' ? 2 : 3)));
    else ch = 0;
    uchar_t *sp = src->data;
    for (i=0; i<total; i++, sp+=s) this->data[i] = sp[ch];
    return true;
  }
  
  bool copyChannelIntoImage(Image *dst, int ch) {
    if (!dst || type != PIXEL_GRAY || !sameSize(dst)) return false;
    int   i, total = w*h, s = dst->pixel_size;
    if      (ch < 4) {}
    else if (dst->type==PIXEL_GRAY)  ch = 0;
    else if (dst->type==PIXEL_GRAYA) ch = (ch=='A' ? 1 : 0);
    else if (dst->type==PIXEL_RGB)   ch = (ch=='R' ? 0 : (ch=='G' ? 1 : 2));
    else if (dst->type==PIXEL_RGBA)  ch = (ch=='R' ? 0 : (ch=='G' ? 1 : (ch=='B' ? 2 : 3)));
    else ch = 0;
    uchar_t *sp = dst->data;
    for (i=0; i<total; i++, sp+=s) sp[ch] = this->data[i];
    return true;
  }
...
public:
  void getImageInfo(char buf[]) {
    ...
    char *type_str;
    switch(type) {
    case PIXEL_GRAY:  type_str = (char*)"GRAY "; break;
    case PIXEL_RGB:   type_str = (char*)"RGB  "; break;
    case PIXEL_BGR:   type_str = (char*)"BGR  "; break;
    case PIXEL_RGBA:  type_str = (char*)"RGBA "; break;
    case PIXEL_GRAYA: type_str = (char*)"GRAYA"; break;
    case PIXEL_YUV:   type_str = (char*)"YUV  "; break;
    case PIXEL_YUYV:  type_str = (char*)"YUYV "; break;
    case PIXEL_INT:   type_str = (char*)"INT  "; break;
    case PIXEL_FLOAT: type_str = (char*)"FLOAT"; break;
    case PIXEL_VOIDP: type_str = (char*)"VOID*"; break;
    default:          type_str = (char*)"---- "; break;
    }
    if (!data) { sprintf( buf, "(%4d x %4d) %s(%d)  data==NULL  ", w, h, type_str, pixel_size); return; }
    int		i, total = w * h, count=0, nonneg=0, *ip;
	...
	for (i=0, ip = (int*)data; i<total; i++) if (ip[i]>=0) nonneg++;
	sprintf(buf,"(%4d x %4d) %s(%d)  min=%3d  max=%3d  NonZero=%d(%d%%) NonNeg=%d(%d%%)", w, h, type_str, pixel_size, mini, maxi, count, count*100/total, nonneg, nonneg*100/total);
      }
      break;
    case PIXEL_FLOAT:
...
#include "imgh_fileio.hpp"	// only for IMGH::ImageFileIO
using namespace std;
