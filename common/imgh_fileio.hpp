...
// -------------------------------------------------------------------
// 
// IMGH::ImageFileIO is the only class in the IMGH library, which depends 
...
//   use IMGH::ImageFileIO in your code.
// 
...
public:  
  ...
  char* getFileBaseName(const char *filename, char *out, bool with_path=false) {
    int  i=0, j, k, len = (int)strlen(filename);
    if (!with_path) { for (i=len-1; i>=0; i--) if (filename[i]=='/') break;  i++; }
...
public:  
  ...
  const char* getFileExtension(const char *filename) {
    int  i;  // find the beginning of the file extension of 'filename'
    for (i=(int)strlen(filename); i>0 && filename[i]!='.'; i--);
    return (i>0 ? filename+i+1 : (const char*)"");
  }
  char* getFileBaseName(const char *filename, char *out, bool with_path=false) {
    int  i=0, j, k, len = (int)strlen(filename);
    if (!with_path) { for (i=len-1; i>=0; i--) if (filename[i]=='/') break;  i++; }
...
private:  
  ...
  Image* readPXMFile(FILE *fp, Image *img) {
	...
	type[0] != 'P' || type[1] < '0' || type[1] > '6' || w==0 || h==0) {
      std::cerr << "Error (IMGH::ImageFileIO::readPXMFile): invalid PXM file " << std::endl;
      return NULL;
    }
    int  i, total, v;
    IMGH::pixel_t pt;
  ...
  bool writePPMFile(FILE *fp, Image *img) {
    if (img->type != PIXEL_RGB) {
      std::cerr << "Error (IMGH::ImageFileIO::writePPMFile): It's not PIXEL_RGB." << std::endl;
      return false;
    }
    char header[80];
  ...
  bool writePGMFile(FILE *fp, Image *img) {
	...
	pc.convRGB2Gray( src, &value );
	fwrite ( &value, sizeof(unsigned char), 1, fp );
      }
    } else {
      std::cerr << "Error (IMGH::ImageFileIO::writePPMFile): It neither PIXEL_RGB nor PIXEL_GRAY." << std::endl;
      return false;
    }
    return true;
  }
...
public:  
  Image* readFile(const char *filename, Image *img=NULL, pixel_t type=PIXEL_UNKNOWN) {
    ...
    if (fp == (FILE*)NULL) {
//       std::cerr << "Error (IMGH::ImageFileIO::readFile): cannot open file '" << filename << "'" << std::endl;
      return NULL;
    }
...
//       std::cerr << "Error (IMGH::ImageFileIO::readFile): invalid file type" << std::endl;  
...
private:  
  ...
  Image* readJPEGFile(FILE *fp, Image *img) {
    ...
    else if (cinfo.output_components == 4) { std::cerr << "Error (IMGH::ImageFileIO::readJPEGFile): ALPHA channel ? " << std::endl; return NULL; }
    else return NULL;
  ...
  bool writeJPEGFile(FILE *fp, Image *img, int quality=90) {
    ...
    default: std::cerr << "Error (IMGH::ImageFileIO::writeJPEGFile): invalid pixel type" << std::endl; return false; 
    }
...
private:  
  ...
  Image* readTIFFFile(FILE *fp, Image *img) {
    ...
    if (tiffp == NULL) {
      std::cerr << "Error (IMGH::ImageFileIO::readTIFFFile): cannot open the file " << std::endl;
      return NULL;
    }
    TIFFRGBAImage tiff;
    char emsg[1024];
...
// 	    img->setRGBA(j, tiff.height-1-i, p[0], p[1], p[2], p[3]);
  ...
  bool writeTIFFFile(FILE *fp, Image *img) {
    std::cerr << "Error (IMGH::ImageFileIO::writeTIFFFile): not implemented yet " << std::endl;
    return false;
  }
...
private:  
  ...
  bool writePNGFile(FILE *fp, Image *img) {
    ...
    case PIXEL_VOIDP: 
      ...
      break;
    default :
      std::cerr << "Error (IMGH::ImageFileIO::writePNGFile): invalid pixel type" << std::endl;
      png_destroy_write_struct(&pngp, &info);
      return false;
    }
