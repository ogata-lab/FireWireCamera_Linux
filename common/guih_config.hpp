...
public:
  int set(const char *section, const char *key, int type, void *p, double defaultv=CFG_UNSET) {
    return set(section, key, type, p, defaultv, CFG_UNSET, CFG_UNSET);
  }
  int set(const char *section, const char *key, int type, void *p, int defaultv) {
    return set(section, key, type, p, (double)defaultv, CFG_UNSET, CFG_UNSET);
  }
  int set(const char *section, const char *key, int type, void *p, char *defaultv) {
    return set(section, key, type, p, (double)(int)defaultv, CFG_UNSET, CFG_UNSET);
  }
  int set(const char *section, const char *key, int type, void *p, double defaultv, double min, double max) {
  ...
  bool processed(const char *section, const char *key) {
    int kidx = findKey(section, key);
    if (kidx >= 0 && keys[kidx].processed) return true;
    return false;
  }
  
  bool process(const char *filename, const char *section=NULL, 
	       bool use_global=false, bool verbose=true) {
  ...
  void printInfo(const char *title=NULL) {
    int  sidx, kidx, i;
    char buf[256];
...
private:
  ...
  int  findKey(const char *section, const char *key, bool use_global=true) {
    return findKey( findSection(section), key, use_global );
  }
  
  int  findKey(int sidx, const char *key, bool use_global=true) {
    int i;
...
public:
  char* makeFullPath(char *fname, char *dir) {
    if (fname == NULL || dir == NULL || fname[0] == '/') return fname;
    char temp[256];
    strcpy( temp, fname );
    strcpy( fname, dir );
    int  len = (int)strlen( fname );
    if (fname[len-1] != '/') { fname[len] = '/'; fname[len+1] = '\0'; }
    strcat( fname, temp );
    return fname;
  }
...
int main(int argc, char **argv) 
{
  ...
  // it wasn't found at all in the configuration file, use processed().
  if (cfg.processed("Width")) printf("'Width' found in the config file\n");
